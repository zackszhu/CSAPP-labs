...
{    
    ...
    symbol_t *newP = (symbol_t*)malloc(sizeof(symbol_t));
    newP->name = name;
    newP->addr = vmaddr;

    /* add the new symbol_t to symbol table */

    newP->next = symtab->next;
    symtab->next = newP;
...
parse_t parse_symbol(char **ptr, char **name)
{
    ...
    // get length
    int length = 0;
    while (IS_LETTER(cur + length) || IS_DIGIT(cur + length))
        length++;
...
parse_t parse_imm(char **ptr, char **name, long *value)
{
    ...
    if (IS_IMM(cur)){
        cur += 1;
        parse_t tmp = parse_digit(&cur, value);
        *ptr = cur;
        return tmp;
    }
...
void immToCode(long *pos, long val){
    *pos = val;
}
...
cont:
    ...
    switch (HIGH(inst->code)) {
      ...
      case I_IRMOVL: {  /* 3:0 Imm, regB - e.g., irmovl $-1, %ebx */
        ...
        immToCode((long*)&y86bin->codes[2], imm);
      ...
      case I_DIRECTIVE: {
        ...
        switch (LOW(inst->code)) {
          case D_DATA: {    /* .long data - e.g., .long 0xC0 */
            ...
            char *name;
            parse_t ret2;
            ret = parse_digit(&cur, &tmp);
            ret2 = parse_symbol(&cur, &name);
            if ((ret == PARSE_ERR) && (ret2 == PARSE_ERR)){
                line->type = TYPE_ERR;
...
int binfile(FILE *out)
{
    ...
    line_t *binLine = y86bin_listhead;
    int pos = 0;

    /* binary write y86 code to output file (NOTE: see fwrite()) */
    while(binLine){
        // check if last non-direction instr
        bool_t last = 1;
        line_t *checkLine = binLine->next;
        while(checkLine){
            if(checkLine->y86bin.bytes != 0){
                last = 0;
                break;
            }
            checkLine = checkLine->next;
        }
        if(pos < binLine->y86bin.addr && !last){
            //fill blanks
            int diff = binLine->y86bin.addr - pos;
            int i;
            for(i = 0; i < diff; i++) fprintf(out, "%c", 0);
            pos = binLine->y86bin.addr;
        }
        size_t num = fwrite(binLine->y86bin.codes, sizeof(byte_t), binLine->y86bin.bytes, out);
        if(num != binLine->y86bin.bytes) return -1;
        pos += binLine->y86bin.bytes;
        binLine = binLine->next;
    }
    return 0;
