...
cc_t compute_cc(alu_t op, long_t argA, long_t argB, long_t val)
{
    ...
    if (op == A_ADD){
        int neg_over = argA < 0 && argB < 0 && val >= 0;
        int pos_over = argA >= 0 && argB >= 0 && val < 0;
        ovf = neg_over || pos_over;
    }
    if (op == A_SUB){
        int neg_over = argB < 0 && argA > 0 && val >= 0;
        int pos_over = argB >= 0 && argA <= 0 && val < 0;
        ovf = neg_over || pos_over;
    }
...
stat_t nexti(y86sim_t *sim)
{
    ...
    byte_t reg = 0; // record the register
    regid_t regA, regB; // record which register

    long_t imm = 0;

    long_t tmp, addr, numA, numB;
    ...
    if ((icode >= 2) && (icode <= 6) || (icode == 10) || (icode == 11)){ 
        // rrmovl, irmovl, rmmovl, mrmovl, opl, pushl, popl
        if (!get_byte_val(sim->m, next_pc, &reg)) {
            err_print("PC = 0x%x, Invalid instruction address", sim->pc);
            return STAT_ADR;
        }
        regA = GET_REGA(reg); // get high bit
        regB = GET_REGB(reg); // get low bit

        next_pc++; // move to next code
    }
    ...
    switch (icode) {
      ...
      case I_RRMOVL:  /* 2:x regA:regB */
        if (!(ifun >= 0 && ifun <= 6)){
            // check the fn
            err_print("PC = 0x%x, Invalid instruction %.2x", sim->pc, ifun);
            return STAT_INS;
        }
        if (!(NORM_REG(regA) && NORM_REG(regB))){
            // check the regs
            err_print("PC = 0x%x, Invalid register %.2x", sim->pc, NORM_REG(regA)? regA : regB);
            return STAT_INS;
        }
      ...
      case I_IRMOVL: /* 3:0 F:regB imm */
        ...
        if (!(NONE_REG(regA) && NORM_REG(regB))){
            // check the regs
            err_print("PC = 0x%x, Invalid register %.2x", sim->pc, NONE_REG(regA)? regA : regB);
            return STAT_INS;
        }
      ...
      case I_ALU: /* 6:x regA:regB */
        ...
        // get A and B
        numA = get_reg_val(sim->r, regA);
        numB = get_reg_val(sim->r, regB);
        // compute and set cc
        imm = compute_alu(ifun, numA, numB);
        sim->cc = compute_cc(ifun, numA, numB, imm);
        set_reg_val(sim->r, regB, imm);
        sim->pc = next_pc;
        break;
      ...
      case I_CALL: /* 8:x imm */
        ...
        if (cond_doit(sim->cc, ifun)){
            // push addr
            tmp = get_reg_val(sim->r, 4);
            tmp -= 4; 
            set_reg_val(sim->r, 4, tmp);
            if (!set_long_val(sim->m, tmp, next_pc)){
      ...
      case I_PUSHL: /* A:0 regA:F */
        ...
        tmp -= 4;
        set_reg_val(sim->r, 4, tmp);
        if (!set_long_val(sim->m, tmp, numA)){
            err_print("PC = 0x%x, Invalid stack address 0x%x", sim->pc, tmp);
            return STAT_ADR;
        }
        sim->pc = next_pc;
      ...
      case I_POPL: /* B:0 regA:F */
        ...
        tmp = get_reg_val(sim->r, 4);
        if (!get_long_val(sim->m, tmp, &numA)){
                err_print("PC = 0x%x, Invalid stack address 0x%x", sim->pc, tmp);
                return STAT_ADR;
            }
        tmp += 4;
        set_reg_val(sim->r, 4, tmp);
        set_reg_val(sim->r, regA, numA);
        sim->pc = next_pc;
        break;
